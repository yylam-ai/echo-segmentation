...
def load_model(cfg:CfgNode, is_gpu: bool = True, device=torch.device("cpu"), kpts_extractor_weights: str = None) -> torch.nn.Module:
    ...
    if model_name == 'CNNGCN_freezed':
        ...
        model = CNN_GCN(kpt_channels=2, gcn_channels=[4, 8, 8, 16, 16, 32, 32, 48], backbone=backbone, num_kpts=num_kpts, is_gpu=is_gpu, device=device)
        model.output_type = 'img2kpts'
