...
class kpts_decoder(nn.Module):
    ...
    def __init__(self, features_size, kpt_channels, gcn_channels, num_kpts=8, tsteps=1, is_gpu=True, device=torch.device("cpu")):
        ...
        # init GCN:
        self.init_gcn(adjacency, features_size, tsteps, is_gpu, device=device)
    ...
    def init_gcn(self, adjacency: np.ndarray, features_size: int, tsteps: int, is_gpu: bool, device=torch.device("cpu")):
        ...
        if not is_gpu:
            ...
            self.spiral_indices = self.spiral_indices.to(self.device)
